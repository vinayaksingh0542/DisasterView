...
TEMP_FIRE_THRESHOLD = float(
    os.getenv("TEMP_FIRE_THRESHOLD", "60.0")
)

MQ9_FIRE_THRESHOLD = float(
    os.getenv("MQ9_FIRE_THRESHOLD", "2000.0")
)

MQ135_SMOKE_THRESHOLD = float(
    os.getenv("MQ135_SMOKE_THRESHOLD", "400.0")
)

FLOOD_CRITICAL_DIST = float(
    os.getenv("FLOOD_CRITICAL_DIST", "20.0")
)

FLOOD_HIGH_DIST = float(
    os.getenv("FLOOD_HIGH_DIST", "50.0")
)

DEBOUNCE_SECONDS = int(
    os.getenv("INCIDENT_DEBOUNCE_SECONDS", "30")
)
...
async def receive_sensor_data(
    ...
    ref_height = (
        device.hcsr04_reference_height_cm
        if device
        else 200.0
    )
    ...
    debounce_cutoff = (
        now - datetime.timedelta(seconds=DEBOUNCE_SECONDS)
    )


    # ------------------------------------------------------------------------
    # Helper function for incident debounce
    # ------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # Flame sensor = primary direct fire indicator
    # -------------------------------------------------------------------------

    if data.flame_detected:

        fire_evidence.append(
            "Flame IR Sensor: TRIGGERED (Active Flame)"
        )


    # -------------------------------------------------------------------------
    # High temperature = supporting fire evidence
    # -------------------------------------------------------------------------

    if (
        data.temperature is not None
        and data.temperature > TEMP_FIRE_THRESHOLD
    ):

        fire_evidence.append(
            f"DHT22 Temperature: {data.temperature:.1f}°C "
            f"(Threshold: >{TEMP_FIRE_THRESHOLD}°C)"
        )


    # -------------------------------------------------------------------------
    # MQ-9 = supporting evidence ONLY when flame or high temperature exists
    #
    # This prevents normal MQ-9 baseline values such as ~1400 from
    # independently generating false FIRE incidents.
    # -------------------------------------------------------------------------
    ...
    if (
        ...
        if (
            data.mq9_gas_level is not None
            and data.mq9_gas_level > MQ9_FIRE_THRESHOLD
        ):

            fire_evidence.append(
                f"MQ-9 Combustible Gas Level: "
    ...
    # -------------------------------------------------------------------------
    # Create FIRE incident if fire evidence exists
    # -------------------------------------------------------------------------
    ...
    if len(fire_evidence) > 0:
        ...
        if not has_recent_incident("FIRE"):
            ...
            # CRITICAL requires flame + another strong supporting indicator
            ...
            incident = Incident(
                ...
                lng=dev_lng,
                description=(
                    f"Multi-sensor fire detection: "
                    f"{len(fire_evidence)} corroborated indicator(s)."
                ),
                source="SENSOR_FUSION",
                evidence_score=evidence_score,
                evidence_details={
                    "sensors": fire_evidence,
                    "device_id": data.device_id
                },
                created_at=now
            )
            ...
            await manager.broadcast(
                '{"type": "NEW_INCIDENT", "data": "FIRE"}'
            )


    # =========================================================================
    # 2. SMOKE SENSOR FUSION
    #    Evaluated only when open flame is NOT present
    # =========================================================================
    ...
    # -------------------------------------------------------------------------
    # MQ-135 smoke / air-quality evidence
    # -------------------------------------------------------------------------
    ...
    if (
        ...
        smoke_evidence.append(
            f"MQ-135 Air Quality / Particulates: "
            f"{data.mq135_air_quality:.0f} "
            f"(Threshold: >{MQ135_SMOKE_THRESHOLD})"
        )
...
if (
    ...
    if (
        len(smoke_evidence) > 0
        and len(fire_evidence) == 0
    ):
        ...
        if not has_recent_incident("SMOKE"):
            ...
            incident = Incident(
                ...
                lng=dev_lng,
                description=(
                    "Elevated hazardous particulates & "
                    "toxic air quality detected."
                ),
                source="SENSOR_FUSION",
                evidence_score=(
                    "MEDIUM"
            ...
            await manager.broadcast(
                '{"type": "NEW_INCIDENT", "data": "SMOKE"}'
            )


    # =========================================================================
    # 3. FLOOD SENSOR FUSION
    ...
    if data.water_distance_cm is not None:
        ...
        water_level = max(
            0.0,
            ref_height - data.water_distance_cm
        )
        ...
        if data.water_distance_cm < FLOOD_CRITICAL_DIST:
            ...
            flood_evidence.append(
                f"HC-SR04 Clearance Distance: "
                f"{data.water_distance_cm:.1f}cm "
                f"(Threshold: <{FLOOD_CRITICAL_DIST}cm)"
            )

            flood_evidence.append(
                f"Estimated Water Height: "
                f"{water_level:.1f}cm above reference datum "
                f"({ref_height:.0f}cm)"
            )


        # ---------------------------------------------------------------------
        # High flood threshold
        # ---------------------------------------------------------------------
            ...
            flood_evidence.append(
                f"HC-SR04 Clearance Distance: "
                f"{data.water_distance_cm:.1f}cm "
                f"(Threshold: <{FLOOD_HIGH_DIST}cm)"
            )
        ...
        if severity is not None:
            ...
            if not has_recent_incident("FLOOD"):
                ...
                incident = Incident(
                    ...
                    lng=dev_lng,
                    description=(
                        f"Rapid water elevation detected by "
                        f"ultrasonic rangefinder "
                        f"({water_level:.1f}cm)."
                    ),
                    source="SENSOR_FUSION",
                    evidence_score=severity,
                    evidence_details={
                        "sensors": flood_evidence,
                        "device_id": data.device_id
                    },
                    created_at=now
                )
                ...
                await manager.broadcast(
                    '{"type": "NEW_INCIDENT", "data": "FLOOD"}'
                )


    # =========================================================================
    # Notify dashboard about new sensor telemetry
    # =========================================================================
...
class IncidentStatusUpdate(BaseModel):
    ...
    status: str = Field(
        ...,
        description="UNVERIFIED, ACKNOWLEDGED, RESOLVED, DISMISSED"
    )
...
# ============================================================================
# AI INFERENCE ENGINE INTEGRATION
# STANDALONE DEMO MODULE - LAZY LOADED
# ============================================================================
...
def get_model_registry():
    ...
    if not inferencer:
        ...
        return {
            ...
            "message": (
                "AI Inferencer module not available."
            ),
...
async def run_inference(
    ...
    results = inferencer.infer_image(
        image_bytes
    )


    # ------------------------------------------------------------------------
    # Broadcast high-confidence software detection
    # ------------------------------------------------------------------------
